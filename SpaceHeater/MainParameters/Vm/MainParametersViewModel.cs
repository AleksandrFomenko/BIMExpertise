...
namespace SpaceHeater.MainParameters.Vm;
...
public partial class MainParametersViewModel : ObservableObject
{
    ...
    public MainParametersViewModel(MainParametersModel model, Document doc, Dispatcher dispatcher)
    {
        ...
        ButtonEnable = true;
        GeomParameters = doc.GetProjectParameters(BuiltInCategory.OST_MechanicalEquipment);
        ...
        {
            new Options2("Все радиаторы/регистры", new GridLength(1, GridUnitType.Star)),
    ...
    partial void OnDiamParameterChanged(string value) => _model.WidthParameter = value;
    ...
    private void Execute()
    {
        ...
        var mainTransaction = new Transaction(_doc, "Space heater Transaction");
        ...
        timer.Tick += (s, e) =>
        {
            ...
            for (int i = 0; i < batchSize && count < elements.Count; i++)
            {
                ...
                {
                    TaskDialog.Show("err", exception.ToString());
                }
